...
$j( document ).ready(function () {                                               
    ...
    var shrinkTimeout,$expand,$heading,$link,$brackets
    $expand=$j('#expand')
    $heading=$j('#expand_heading')
    $link=$expand.find(".expander").eq(0)
    ...
    $j(".content").siblings().addBack().contents().addBack().on("mouseenter", function (event) {
        ...
        var $vis = $j(that).hasClass("content") || $j(that).hasClass("expander") ? $title : $not_content 
    ...
    // handler for click on item menu in .content divs 
    ...
    // handler hooked on click on title that updates data stored in content 
    ...
    // following functions for expand all button animation
